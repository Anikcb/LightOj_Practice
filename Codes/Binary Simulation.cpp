...
void init(ll node, ll b, ll e)
{
    if (b == e)
    {
        if(arr[b]==1)tree[node].sum[1] = 1;
        else tree[node].sum[0] = 1;
        return;
    }
    ll Left = node * 2;
    ll Right = node * 2 + 1;
...
{
    if(b!=e){
        tree[node*2].prop=(tree[node*2].prop + tree[node].prop)%2;
        tree[node*2+1].prop=(tree[node*2+1].prop + tree[node].prop)%2;
    }
    if(tree[node].prop%2==1){
            sw1(tree[node].sum[0],tree[node].sum[1]);
    }
    tree[node].prop=0;
    return;
}
...
{
    if(tree[node].prop)fun(node,b,e);
    if (i > e || j < b)return;
    if (b >= i && e <= j)
    {
        if(b!=e){
            tree[node*2].prop=(tree[node*2].prop + 1)%2;
            tree[node*2+1].prop=(tree[node*2+1].prop + 1)%2;
        }
        sw1(tree[node].sum[0],tree[node].sum[1]);
        return;
    }
...
int main()
{
    ...
    for(ll tt=1; tt<=tst; tt++)
    {
        ...
        while(m--)
        {
            ...
            else cout<<query(1,1,n,b,b)<<endl;
        }
        memz(tree);
