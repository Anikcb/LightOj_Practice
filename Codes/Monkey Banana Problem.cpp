...
ll Coin(ll x, ll y, ll px, ll py)
{
    if( px>=1 && px<=x && py>=1 && py<=y){
        if(dp[px][py]!=-1)return dp[px][py];
        ll val=0;
        val=max(val, Coin(x,y,px+1,py)+A[px][py]);
        val=max(val,Coin(x,y,px+1,py+1)+A[px][py]);
        return dp[px][py] = val;
    }
    return 0;
}
...
int main()
{
    ...
    for(ll tt=1; tt<=tst; tt++)
    {
        ...
        for(ll i=n-1,k=n+1, l=2;i>=1;i--,k++,l++)
        {
            rep(j,l,l+i)cin>>A[k][j];
        }

        cout<<"Case "<<tt<<": ";
        cout<<Coin(2*n, n, 1, 1)<<endl;
