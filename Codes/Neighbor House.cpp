...
ll knapsack(ll i, ll j, ll n)
{
    if(i>n)return 0;
    if(j<1 || j>3)return 1e13;
    ll val=1e12;
    if(dp[i][j]!=-1)return dp[i][j];
    val = min(val, knapsack(i+1, j+1, n)+A[i][j]);
    val = min(val, knapsack(i+1, j+2, n)+A[i][j]);
    val = min(val, knapsack(i+1, j-1, n)+A[i][j]);
    val = min(val, knapsack(i+1, j-2, n)+A[i][j]);
    return dp[i][j]=val;
}
...
int main()
{
    ...
    for(ll tt=1; tt<=tst; tt++)
    {
        ...
        ll val=1e12;
        val=min(val, knapsack(1,1,n));
        val=min(val, knapsack(1,2,n));
        val=min(val, knapsack(1,3,n));
        cout<<"Case "<<tt<<": ";
        cout<<val<<endl;
