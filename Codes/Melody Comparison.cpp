...
pll stringMatching()
{
    ...
    if (strncmp(T + SA[lo], P, m) != 0) return MP(-1, -1);
    pll ans;
    ans.first = lo;
    lo = 0;
    hi = n - 1;
...
//    IOS;
    ...
    for(ll tt=1; tt<=tst; tt++)
    {
        ...
        constructSA();
        computeLCP();
        pll len=stringMatching();
        if(len.ff!=-1)
        {
            rep(i,len.ff,len.ss+1)q.push(SA[i]+m-1);
        }
        for(ll i=0;i<n;i++)
        {
            if(q.empty())arr[i]=n-1;
            else
            {
                arr[i]=q.top();
                if(i>q.top()-m+1)q.pop();
            }
        }

        rep(i,1,n)
        {
            if(SA[i]+LCP[i] < arr[SA[i]+1]) res+=(arr[SA[i]+1]-SA[i]-LCP[i]);
        }
