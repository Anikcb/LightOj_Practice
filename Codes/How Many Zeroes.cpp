...
ll DP(ll isSmall, ll pos, ll start, ll value)
{
    if(pos==len)return value;
    if(dp[isSmall][pos][start][value]!=-1)return dp[isSmall][pos][start][value];

    ll range = (isSmall==1?9:v[pos]),res=0,st;
    if(start)st=1;
    else st=0;
    for(ll i=st; i<=range; i++){
        res+=DP(isSmall | i<v[pos], pos+1, 0, (i==0)+value);
    }
    if(start)res+=DP(1,pos+1,1,0);

    return dp[isSmall][pos][start][value]=res;
}
