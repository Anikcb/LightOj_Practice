...
ll NcR(ll n, ll r)
{
    ll val=(bigmod(fact[n-r],MOD-2,MOD)*bigmod(fact[r],MOD-2,MOD))%MOD;
    val=(val*fact[n])%MOD;
    re val;
}
        ...
        else
        {
            ...
            foR0(len)
            {
               ...
               while(val && j<k)
                ...
                if(n-cnt+k-1<k-1)continue;
                if(h&1)res=(res-NcR(n-cnt+k-1,k-1))%MOD;
                else res=(res+NcR(n-cnt+k-1,k-1))%MOD;
            }
            res+=MOD;
            cout<<res%MOD<<endl;
        }
