...
ll find_set(ll n)
{
    return (parent[n]==n?n : parent[n]=find_set(parent[n]));
}

void min_spanning_tree(ll n)
{
    foR1(n)parent[i]=i;
    sort(all(e));
    ll cnt=0;
    foR0(SZ(e))
    {
        ...
        if(a!=b){
            cnt++;
            parent[a]=b;
            tre[e[i].u].pb({e[i].v , e[i].w});
            tre[e[i].v].pb({e[i].u , e[i].w});
            if(cnt==n-1)break;
        }
    }
}
