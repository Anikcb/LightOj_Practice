...
ll knapsack(ll cnt, ll n, ll sum, ll W, ll i)
{
    if(i>n || cnt>B[i])return 0;
    if(sum>=W){
        if(sum == W)return 1;
        else return 0;
    }
    ...
    ll x = knapsack(cnt+1, n, sum+A[i], W, i) % 100000007;
    ll y = knapsack(0, n, sum, W, i+1) % 100000007;

    return dp[i][cnt][sum] = (x+y) % 100000007;
}
